...
from rq1_concrete_replay_store import (  # noqa: E402
    MANIFEST_NAME, STORE_DIR, ReplayPersistenceError, _atomic_json,
    _entry_test_keys, _oracle_binding_errors, _storage_slot_recovery_errors,
    annotate_generalization, audit_manifest,
    deterministic_replay_errors, deterministic_replay_oracles,
...
def main() -> int:
    ...
    storage_source = (
        'contract R { function test_cov_0() public { c0.Initialized(); '
    ...
    storage_oracle = [{
        ...
        "observed": "observed", "expected": "uint256(1)",
        "provenance": "stage2-witness", "target_receiver": "c0",
        "storage_variable": "intitalized", "storage_slot": 2,
        "storage_offset_bytes": 20, "storage_width_bytes": 1,
        "assertion": "assertEq(observed, uint256(1));",
    }]
    bad += check(not _oracle_binding_errors(
        storage_source, "test_cov_0", "Initialized", storage_oracle),
    ...
    for field, value in (("storage_slot", 3), ("storage_offset_bytes", 19),
        ...
        bad += check(bool(_oracle_binding_errors(
            storage_source, "test_cov_0", "Initialized", wrong)),
                     f"storage-slot oracle rejects wrong {field}")
    delayed_storage = storage_source.replace(
        "uint256 observed =", "uint256 unrelated = 0; uint256 observed =")
    bad += check(bool(_oracle_binding_errors(
        delayed_storage, "test_cov_0", "Initialized", storage_oracle)),
                 "storage-slot oracle rejects a non-immediate read")
    storage_original = storage_source.replace(
        'uint256 observed = (uint256(vm.load(address(c0), bytes32(uint256(2)))) '
        '>> 160) & uint256(0xff); assertEq(observed, uint256(1)); ', '')
    bad += check(not _storage_slot_recovery_errors(
        storage_source, storage_original, "test_cov_0", "Initialized",
        storage_oracle[0]),
                 "storage-slot recovery proves an assertion-only augmentation")
    bad += check(bool(_storage_slot_recovery_errors(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        revert_oracles, revert_errors = deterministic_replay_oracles(
            deploy_revert, "test_cov_ctor_revert", "__deploy__")
        bad += check(not revert_errors and revert_oracles == [{
            "class": "R0", "kind": "revert", "source": "expectRevert"
        }], "constructor expectRevert is detected as a concrete execution oracle")
        bad += check(not _oracle_binding_errors(
            deploy_revert.read_text(), "test_cov_ctor_revert", "__deploy__",
            revert_oracles), "constructor expectRevert binds to the deploy replay")
    ...
    for inert_binding in (
        ...
        bad += check(any("exact typed" in error for error in
                         _oracle_binding_errors(
                             inert_binding, "test_cov_0", "decimals",
                             return_oracle)),
    ...
    tuple_oracles = [{
        ...
        "return_index": index, "return_arity": 3, "observed": observed,
        "expected": expected, "provenance": "stage2-witness",
        "target_receiver": "c0", "assertion": assertion,
        ...
        ("uint256", "startedAt", "uint256(0)",
         "assertEq(startedAt, uint256(0));"),
    ))]
    bad += check(not _oracle_binding_errors(
        tuple_source, "test_cov_0", "latestRoundData", tuple_oracles),
                 "complete typed tuple return binds every fixed witness component")
    bad += check(any("every ABI return component" in error for error in
                     _oracle_binding_errors(
                         tuple_source, "test_cov_0", "latestRoundData",
                         tuple_oracles[:-1])),
    ...
    bad += check(bool(_oracle_binding_errors(
        boolean_state_source.replace("assertTrue(c0.", "assertTrue(helper."),
        "test_cov_0", "setApprovalForAll", wrong_state_receiver)),
                 "a boolean state oracle rejects another receiver")
    another_getter = [{**boolean_state_oracle[0],
                       "observed": "c0.other(address(this), operator)"}]
    bad += check(bool(_oracle_binding_errors(
        boolean_state_source, "test_cov_0", "setApprovalForAll", another_getter)),
                 "a boolean state oracle rejects another getter")
    self_state = [{**boolean_state_oracle[0],
                   "assertion": ("assertEq(c0.isApprovedForAll(address(this), operator), "
                                 "c0.isApprovedForAll(address(this), operator));")}]
    bad += check(bool(_oracle_binding_errors(
        boolean_state_source.replace(
            "assertTrue(c0.isApprovedForAll(address(this), operator));",
            self_state[0]["assertion"]),
        "test_cov_0", "setApprovalForAll", self_state)),
    ...
    assertion_before_call = boolean_state_source.replace(
        ...
        "assertTrue(c0.isApprovedForAll(address(this), operator)); "
        "c0.setApprovalForAll(operator, true);")
    bad += check(any("immediate exact" in error for error in _oracle_binding_errors(
        assertion_before_call, "test_cov_0", "setApprovalForAll",
        boolean_state_oracle)),
                 "a boolean state assertion before the target call is rejected")
    compound_state = [{**boolean_state_oracle[0],
                       "observed": ("c0.isApprovedForAll(address(this), operator) == "
                                    "c0.isApprovedForAll(address(this), operator)"),
                       "assertion": ("assertTrue(c0.isApprovedForAll(address(this), operator) == "
                                     "c0.isApprovedForAll(address(this), operator));")}]
    bad += check(bool(_oracle_binding_errors(
        boolean_state_source.replace(boolean_state_oracle[0]["assertion"],
                                     compound_state[0]["assertion"]),
        "test_cov_0", "setApprovalForAll", compound_state)),
    ...
    commented_call = (
        ...
        'c0.setApprovalForAll(operator, true); '
        'assertTrue(c0.isApprovedForAll(address(this), operator)); } }')
    helper_oracle = [{**boolean_state_oracle[0], "target_receiver": "helper",
                      "observed": "helper.isApprovedForAll(address(this), operator)",
                      "assertion": "assertTrue(helper.isApprovedForAll(address(this), operator));"}]
    bad += check(bool(_oracle_binding_errors(
        commented_call, "test_cov_0", "setApprovalForAll", helper_oracle)),
    ...
    second_target_call = boolean_state_source.replace(
        ...
        "assertTrue(c0.isApprovedForAll(address(this), operator));")
    bad += check(bool(_oracle_binding_errors(
        second_target_call, "test_cov_0", "setApprovalForAll",
        boolean_state_oracle)),
    ...
    string_comment_delimiters = boolean_state_source.replace(
        ...
        "c0.setApprovalForAll(operator, true);")
    bad += check(bool(_oracle_binding_errors(
        string_comment_delimiters, "test_cov_0", "setApprovalForAll",
        boolean_state_oracle)),
                 "comment delimiters inside strings cannot hide another target call")
    adjacent_helper = (
        'contract R { function test_cov_0() public { string memory s = "{"; } '
        'function helper() public { c0.setApprovalForAll(operator, true); '
        'assertTrue(c0.isApprovedForAll(address(this), operator)); } }')
    bad += check(bool(_oracle_binding_errors(
        adjacent_helper, "test_cov_0", "setApprovalForAll", boolean_state_oracle)),
    ...
    try_event_source = event_source.replace(
        "c0.f(address(uint160(7)), 9);",
        "try c0.f(address(uint160(7)), 9) {} catch {}")
    bad += check(not _oracle_binding_errors(
        try_event_source, "test_cov_0", "f", event_oracle),
                 "an exact event assertion makes an empty try/catch replay strict")
    nonempty_event_catch = try_event_source.replace(
        "catch {}", "catch { helper(); }")
    bad += check(any("catch body" in error for error in _oracle_binding_errors(
        nonempty_event_catch, "test_cov_0", "f", event_oracle)),
                 "an event oracle rejects behavior in the catch branch")
    nonempty_event_success = try_event_source.replace(
        ") {} catch", ") { helper(); } catch")
    bad += check(any("success body" in error for error in _oracle_binding_errors(
        nonempty_event_success, "test_cov_0", "f", event_oracle)),
                 "an event oracle rejects behavior in the try success branch")
    wrong_event_emitter = [{**event_oracle[0], "expected": {
        **event_oracle[0]["expected"], "emitter": "address(helper)"}}]
    bad += check(bool(_oracle_binding_errors(
        event_source, "test_cov_0", "f", wrong_event_emitter)),
                 "an event from another emitter cannot certify the target call")
    missing_event_topic = [{**event_oracle[0],
                            "assertion": event_assertions.replace(
                                "assertEq(_veriputLogs[0].topics[1], "
                                "bytes32(uint256(uint160(address(uint160(7))))));", "")}]
    bad += check(any("exact emitter" in error for error in _oracle_binding_errors(
        event_source.replace(
            "assertEq(_veriputLogs[0].topics[1], "
            "bytes32(uint256(uint160(address(uint160(7))))));", ""),
        "test_cov_0", "f", missing_event_topic)),
                 "an event oracle cannot omit an indexed topic assertion")
    extra_event_call = event_source.replace(
        "vm.recordLogs();", "vm.recordLogs(); c0.f(address(uint160(7)), 9);")
    bad += check(any("exactly one" in error for error in _oracle_binding_errors(
        extra_event_call, "test_cov_0", "f", event_oracle)),
    ...
    bad += check(any("immediately closed" in error for error in
                     _oracle_binding_errors(
                         late_event_read, "test_cov_0", "f", event_oracle)),
                 "an intervening call cannot enter the recorded event window")
    early_event_helper = event_source.replace(
        "vm.recordLogs();", "vm.recordLogs(); helper();")
    bad += check(any("immediately closed" in error for error in
                     _oracle_binding_errors(
                         early_event_helper, "test_cov_0", "f", event_oracle)),
                 "a helper cannot emit the asserted event before the target call")
    self_referential_event = [{**event_oracle[0], "expected": {
        **event_oracle[0]["expected"],
        "topics": ["_veriputLogs[0].topics[0]"],
        "data": "_veriputLogs[0].data",
    }}]
    bad += check(bool(_oracle_binding_errors(
        event_source, "test_cov_0", "f", self_referential_event)),
                 "event expectations cannot compare observed logs to themselves")
    injected_event_expression = [{**event_oracle[0], "expected": {
        **event_oracle[0]["expected"],
        "data": 'abi.encode(uint256(9)), "message"',
    }}]
    bad += check(bool(_oracle_binding_errors(
        event_source, "test_cov_0", "f", injected_event_expression)),
                 "event expectations reject top-level expression injection")
    trailing_event_action = event_source.replace(
        event_assertions + " } }", event_assertions + " helper(); } }")
    bad += check(any("unbound statements" in error for error in
                     _oracle_binding_errors(
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        bad += check(not deterministic_replay_errors(
            event_gate, "test_cov_0", "f"),
        ...
        original_event.write_text(
            "contract R { function test_cov_0() public { "
            "c0.f(address(uint160(7)), 9); } }")
        recovered_event = root / "producer" / "test" / "RecoveredEventReplay.t.sol"
        recovered_event.write_text(
            'import {Vm} from "forge-std/Vm.sol"; ' + event_source)
        recovered_event_row = {
            **concrete, "file": str(recovered_event),
            "recovered_from_file": str(original_event),
            "concrete_oracles": event_oracle,
        }
        recovered_event_entry = persist_concrete_replay(
            subject, recovered_event_row, dry_run=True)
        bad += check(
            recovered_event_entry["origin"]["recovered_from_test_sha256"],
        ...
        typed_source, typed_oracles = add_concrete_normal_exit_oracle(
            typed_input, "test_cov_0", "f")
        typed_try.write_text(typed_source)
        bad += check(typed_oracles and
                     typed_oracles[0]["target_receiver"] == "c",
                     "producer emits exact call-options receiver provenance")
        bad += check(not deterministic_replay_errors(
            typed_try, "test_cov_0", "f"),
        ...
        typed_row = {**concrete, "file": str(typed_project),
                     "concrete_oracles": typed_oracles}
        ...
        returning_try = typed_try.read_text().replace(
            'catch (bytes memory data) {}',
        ...
        try:
            ...
            bad += check(False,
                         "a catch return that bypasses the assertion is rejected")
        except ReplayPersistenceError:
            pass
        producer_source = (
            'contract Produced {\n  function test_cov_0() public {\n'
        ...
        low_level_row = {**concrete, "file": str(produced_project),
        ...
        wrong_status = [{**low_level_oracles[0], "observed": "other",
        ...
        fallback_oracles = [{
            "class": "R0", "kind": "call-status", "observed": "ok",
            "expected": True, "provenance": "stage2-witness",
            "target_receiver": "c",
            "assertion": 'assertTrue(ok, "fallback call must complete");',
        }]
        produced_project.write_text(fallback_source)
        fallback_row = {**concrete, "unit": "fallback", "file": str(produced_project),
        ...
        fallback_row["concrete_oracles"] = [{**fallback_oracles[0],
                                             "target_receiver": "helper"}]
        try:
            ...
            pass
        produced_project.write_text(fallback_source.replace(
            '    assertTrue(ok, "fallback call must complete");',
        ...
        bad += check(generalized["same_path_candidates"] == 1
        ...
        manifest = load_manifest(subject)
        manifest["entries"][0]["origin"]["stage2_witness_check"] = (
            "CERTIFIED-BASIS-REPLAY")
        manifest["entries"][0]["origin"]["stage2_source"] = (
            "certified-region-concrete-fallback")
        _atomic_json(subject / STORE_DIR / MANIFEST_NAME, manifest)
        generalized = annotate_generalization(subject, [valid_put, concrete])
        classified = load_manifest(subject)["entries"][0]
        bad += check(generalized["confirmed_generalized_to_put"] == 1
                     and classified["generalization_status"] ==
        ...
        coverage = persistence_coverage(
            [valid_put, concrete], load_manifest(subject)["entries"], subject)
        ...
        invalid_entry_coverage = persistence_coverage(
            [valid_put, concrete], [{**manifest["entries"][0],
                                     "forge_log_sha256": "bad"}], subject)
        bad += check(
        ...
        duplicate_name_coverage = persistence_coverage(
            [concrete, second_concrete], load_manifest(subject)["entries"], subject)
        ...
        summary.write_text(json.dumps({"deliverable_b": {"rows": [{
            "kind": "concrete", "test": evidence_entry["test"],
            "unit": evidence_entry["origin"]["unit"],
            "enc": evidence_entry["origin"]["enc"],
            "file": str(copied_test), "forge_status": "Success",
        ...
        bad += check(any("execution evidence" in error for error in
                         audit_manifest(subject, {"entries": [bad_summary]})),
