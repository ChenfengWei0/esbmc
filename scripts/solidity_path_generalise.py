...
def main():
    ...
    ap.add_argument("--claim-budget", type=int, default=0,
                    help="cap the number of CLAIMS an outer-box round emits, "
                         ...
                         "quantity.\n"
                         "The RATE is now measured: ~0.26s per claim on this "
                         "machine, of which only ~0.05s is the solver -- the "
                         "rest is the per-claim slice-and-encode inside "
                         "multi-property. GOTO creation and symex are FLAT in "
                         "the claim count (1.17s / 0.10s at 4, 16 and 64 "
                         "claims), so it is not instrumentation either.\n"
                         "So a round with a wall budget of T seconds affords "
                         "roughly T/0.26 claims -- about 1150 for 300s. That "
                         "figure is per-MACHINE, which is why it is written "
                         "here and not hard-coded as a default: measure it on "
                         "yours (vary one coordinate's value count, watch the "
                         "total wall) rather than inheriting mine."
                    )
