...
// Contract: LAAR
// Pair:     initReserveWallet vs setClaimLimit
// Mode:     race
...
// ===== TOD harness =====
// ----- initReserveWallet vs setClaimLimit -----
// Shared state variables (touched by both):
//   - claimLimits
//   - balances
contract TOD_initReserveWallet_setClaimLimit {
    function test(
        LAAR c1,
        LAAR c2,
        address a__reserveWallet,
        address b__address, uint256 b__days
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs initReserveWallet then setClaimLimit
        try c1.initReserveWallet(a__reserveWallet) {} catch {}
        try c1.setClaimLimit(b__address, b__days) {} catch {}

        // Order 2: c2 runs setClaimLimit then initReserveWallet
        try c2.setClaimLimit(b__address, b__days) {} catch {}
        try c2.initReserveWallet(a__reserveWallet) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.claimLimits(a__reserveWallet) == c2.claimLimits(a__reserveWallet));
        __tod_race_check(c1.claimLimits(b__address) == c2.claimLimits(b__address));
        __tod_race_check(c1.claimLimits(address(this)) == c2.claimLimits(address(this)));
        __tod_race_check(c1.balances(a__reserveWallet) == c2.balances(a__reserveWallet));
        __tod_race_check(c1.balances(b__address) == c2.balances(b__address));
