...
// Contract: LAAR
// Pair:     decreaseApproval vs transferFrom
// Mode:     race
...
// ===== TOD harness =====
// ----- decreaseApproval vs transferFrom -----
// Shared state variables (touched by both):
//   - balances
contract TOD_decreaseApproval_transferFrom {
    function test(
        LAAR c1,
        LAAR c2,
        address a__spender, uint256 a__subtractedValue,
        address b__from, address b__to, uint256 b__value
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs decreaseApproval then transferFrom
        try c1.decreaseApproval(a__spender, a__subtractedValue) {} catch {}
        try c1.transferFrom(b__from, b__to, b__value) {} catch {}

        // Order 2: c2 runs transferFrom then decreaseApproval
        try c2.transferFrom(b__from, b__to, b__value) {} catch {}
        try c2.decreaseApproval(a__spender, a__subtractedValue) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.balances(a__spender) == c2.balances(a__spender));
