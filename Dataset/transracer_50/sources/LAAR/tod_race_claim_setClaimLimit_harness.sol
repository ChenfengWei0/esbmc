...
// ===== TOD harness =====
// ----- claim vs setClaimLimit -----
// Shared state variables (touched by both):
//   - claimLimits
//   - balances
contract TOD_claim_setClaimLimit {
    function test(
        LAAR c1,
        LAAR c2,
        address a__recipient,
        address b__address, uint256 b__days
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs claim then setClaimLimit
        try c1.claim(a__recipient) {} catch {}
        try c1.setClaimLimit(b__address, b__days) {} catch {}

        // Order 2: c2 runs setClaimLimit then claim
        try c2.setClaimLimit(b__address, b__days) {} catch {}
        try c2.claim(a__recipient) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.claimLimits(a__recipient) == c2.claimLimits(a__recipient));
        ...
        __tod_race_check(c1.balances(a__recipient) == c2.balances(a__recipient));
