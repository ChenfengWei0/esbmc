...
// Contract: LAAR
// Pair:     transferFrom vs transferOwnership
// Mode:     race
...
// ===== TOD harness =====
// ----- transferFrom vs transferOwnership -----
// Shared state variables (touched by both):
//   - owner
//   - balances
contract TOD_transferFrom_transferOwnership {
    function test(
        LAAR c1,
        LAAR c2,
        address a__from, address a__to, uint256 a__value,
        address b__newOwner
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs transferFrom then transferOwnership
        try c1.transferFrom(a__from, a__to, a__value) {} catch {}
        try c1.transferOwnership(b__newOwner) {} catch {}

        // Order 2: c2 runs transferOwnership then transferFrom
        try c2.transferOwnership(b__newOwner) {} catch {}
        try c2.transferFrom(a__from, a__to, a__value) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.owner() == c2.owner());
        __tod_race_check(c1.balances(a__from) == c2.balances(a__from));
        __tod_race_check(c1.balances(a__to) == c2.balances(a__to));
