...
// Contract: LAAR
// Pair:     setClaimLimit vs transferFrom
// Mode:     race
...
// ===== TOD harness =====
// ----- setClaimLimit vs transferFrom -----
// Shared state variables (touched by both):
//   - claimLimits
//   - balances
contract TOD_setClaimLimit_transferFrom {
    function test(
        LAAR c1,
        LAAR c2,
        address a__address, uint256 a__days,
        address b__from, address b__to, uint256 b__value
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs setClaimLimit then transferFrom
        try c1.setClaimLimit(a__address, a__days) {} catch {}
        try c1.transferFrom(b__from, b__to, b__value) {} catch {}

        // Order 2: c2 runs transferFrom then setClaimLimit
        try c2.transferFrom(b__from, b__to, b__value) {} catch {}
        try c2.setClaimLimit(a__address, a__days) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.claimLimits(a__address) == c2.claimLimits(a__address));
        __tod_race_check(c1.claimLimits(b__from) == c2.claimLimits(b__from));
        __tod_race_check(c1.claimLimits(b__to) == c2.claimLimits(b__to));
        __tod_race_check(c1.claimLimits(address(this)) == c2.claimLimits(address(this)));
        __tod_race_check(c1.balances(a__address) == c2.balances(a__address));
        __tod_race_check(c1.balances(b__from) == c2.balances(b__from));
        __tod_race_check(c1.balances(b__to) == c2.balances(b__to));
