...
// Contract: LAAR
// Pair:     transferOwnership vs transferTo
// Mode:     race
...
// ===== TOD harness =====
// ----- transferOwnership vs transferTo -----
// Shared state variables (touched by both):
//   - owner
//   - balances
contract TOD_transferOwnership_transferTo {
    function test(
        LAAR c1,
        LAAR c2,
        address a__newOwner,
        ...
        // Order 1: c1 runs transferOwnership then transferTo
        try c1.transferOwnership(a__newOwner) {} catch {}
        try c1.transferTo(b__address, b__tokens) {} catch {}

        // Order 2: c2 runs transferTo then transferOwnership
        try c2.transferTo(b__address, b__tokens) {} catch {}
        try c2.transferOwnership(a__newOwner) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.owner() == c2.owner());
        __tod_race_check(c1.balances(a__newOwner) == c2.balances(a__newOwner));
