...
// Contract: LAAR
// Pair:     setClaimLimit vs transferOwnership
// Mode:     race
...
// ===== TOD harness =====
// ----- setClaimLimit vs transferOwnership -----
// Shared state variables (touched by both):
//   - owner
//   - claimLimits
//   - balances
contract TOD_setClaimLimit_transferOwnership {
    function test(
        LAAR c1,
        LAAR c2,
        ...
        // Order 1: c1 runs setClaimLimit then transferOwnership
        try c1.setClaimLimit(a__address, a__days) {} catch {}
        try c1.transferOwnership(b__newOwner) {} catch {}

        // Order 2: c2 runs transferOwnership then setClaimLimit
        try c2.transferOwnership(b__newOwner) {} catch {}
        try c2.setClaimLimit(a__address, a__days) {} catch {}

        // Race check: if any shared state differs the pair is order-dependent
        __tod_race_check(c1.owner() == c2.owner());
        __tod_race_check(c1.claimLimits(a__address) == c2.claimLimits(a__address));
        __tod_race_check(c1.claimLimits(b__newOwner) == c2.claimLimits(b__newOwner));
        ...
        __tod_race_check(c1.balances(b__newOwner) == c2.balances(b__newOwner));
