...
// Contract: LAAR
// Pair:     increaseApproval vs transferFrom
// Mode:     race
...
// ===== TOD harness =====
// ----- increaseApproval vs transferFrom -----
// Shared state variables (touched by both):
//   - balances
contract TOD_increaseApproval_transferFrom {
    function test(
        LAAR c1,
        LAAR c2,
        address a__spender, uint256 a__addedValue,
        address b__from, address b__to, uint256 b__value
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs increaseApproval then transferFrom
        try c1.increaseApproval(a__spender, a__addedValue) {} catch {}
        try c1.transferFrom(b__from, b__to, b__value) {} catch {}

        // Order 2: c2 runs transferFrom then increaseApproval
        try c2.transferFrom(b__from, b__to, b__value) {} catch {}
        try c2.increaseApproval(a__spender, a__addedValue) {} catch {}
