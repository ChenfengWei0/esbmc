...
// Contract: LAAR
// Pair:     approve vs decreaseApproval
// Mode:     race
...
// ===== TOD harness =====
// ----- approve vs decreaseApproval -----
// Shared state variables (touched by both):
//   <none — assertion is trivially true>
contract TOD_approve_decreaseApproval {
    function test(
        LAAR c1,
        LAAR c2,
        address a__spender, uint256 a__tokens,
        address b__spender, uint256 b__subtractedValue
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs approve then decreaseApproval
        try c1.approve(a__spender, a__tokens) {} catch {}
        try c1.decreaseApproval(b__spender, b__subtractedValue) {} catch {}

        // Order 2: c2 runs decreaseApproval then approve
        try c2.decreaseApproval(b__spender, b__subtractedValue) {} catch {}
