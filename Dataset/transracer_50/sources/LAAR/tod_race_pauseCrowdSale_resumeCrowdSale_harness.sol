...
// Contract: LAAR
// Pair:     pauseCrowdSale vs resumeCrowdSale
// Mode:     race
...
// ===== TOD harness =====
// ----- pauseCrowdSale vs resumeCrowdSale -----
// Shared state variables (touched by both):
//   - stopped
contract TOD_pauseCrowdSale_resumeCrowdSale {
    function test(
        LAAR c1,
        LAAR c2
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs pauseCrowdSale then resumeCrowdSale
        c1.pauseCrowdSale();
        c1.resumeCrowdSale();

        // Order 2: c2 runs resumeCrowdSale then pauseCrowdSale
        c2.resumeCrowdSale();
        c2.pauseCrowdSale();
