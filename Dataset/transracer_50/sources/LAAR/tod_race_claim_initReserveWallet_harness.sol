...
// Contract: LAAR
// Pair:     claim vs initReserveWallet
// Mode:     race
...
// ===== TOD harness =====
// ----- claim vs initReserveWallet -----
// Shared state variables (touched by both):
//   - balances
contract TOD_claim_initReserveWallet {
    function test(
        LAAR c1,
        LAAR c2,
        address a__recipient,
        address b__reserveWallet
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs claim then initReserveWallet
        try c1.claim(a__recipient) {} catch {}
        try c1.initReserveWallet(b__reserveWallet) {} catch {}

        // Order 2: c2 runs initReserveWallet then claim
        try c2.initReserveWallet(b__reserveWallet) {} catch {}
        ...
        __tod_race_check(c1.balances(b__reserveWallet) == c2.balances(b__reserveWallet));
