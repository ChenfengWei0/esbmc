...
// Contract: LAAR
// Pair:     setClaimLimit vs transferTo
// Mode:     race
...
// ===== TOD harness =====
// ----- setClaimLimit vs transferTo -----
// Shared state variables (touched by both):
//   - claimLimits
//   - balances
contract TOD_setClaimLimit_transferTo {
    function test(
        LAAR c1,
        LAAR c2,
        address a__address, uint256 a__days,
        address b__address, uint256 b__tokens
    ) public {
        require(address(c1) != address(c2), "isolate c1/c2");
        // Order 1: c1 runs setClaimLimit then transferTo
        try c1.setClaimLimit(a__address, a__days) {} catch {}
        try c1.transferTo(b__address, b__tokens) {} catch {}

        // Order 2: c2 runs transferTo then setClaimLimit
        try c2.transferTo(b__address, b__tokens) {} catch {}
