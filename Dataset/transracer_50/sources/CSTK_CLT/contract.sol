...
contract CSTKDropToken is ERC20, Owned {
  ...
  // Constructor
  // ------------------------------------------------------------------------
  constructor(string memory _symbol, string memory _name, uint256 _supply, uint256 _decimals, address _token) {
    symbol = _symbol;
    name = _name;
    decimals = _decimals;
    token = _token;
  ...
  // ------------------------------------------------------------------------
  function allowance(address tokenOwner, address spender) public view virtual override returns (uint256 remaining) {
    tokenOwner;
    spender;
    return uint256(0);
  }
