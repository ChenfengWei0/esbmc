...
| 16 | `revert` / `require` / failed `.transfer()` | `solidity_convert_modifier.cpp::build_revert_rollback_block` (B1) + `solidity_convert_stmt.cpp::TryStatement` if/else nondet split | UNDER for state, OVER for control flow | Public/external reverts run B1's `*this = _sol_save_this; return;`. Internal/private reverts and transfer-failure paths still emit `__ESBMC_assume(false)`. Try/catch is modelled as `if (nondet) { call; success } else { catch }` — catch sits in the else branch so SSA branching keeps success-arm writes out of catch's view (T2.2 confirmed empirically). Cross-iteration in the dispatcher harness is also covered by SSA branching at the while-loop join + path pruning (B5-B confirmed empirically, 2026-04-30). | None for try/catch state rollback or cross-iteration leak via internal-helper revert (closed by SSA branching + B1, regression-locked by `try_catch_*_rollback_pass` and `cross_iter_internal_revert_*_pass`). | Standalone `addr.transfer()` revert outside try/catch still uses `__ESBMC_assume(false)` so a path-pruned write would survive into other paths via SSA merge — orthogonal sub-bug, deferred. |
