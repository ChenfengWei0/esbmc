...
| 3 | ABI encode | `solidity_abi.c` (`abi_encode*`, `abi_encodeCall`, `abi_encodeWithSelector`, `abi_encodeWithSignature`, `abi_encodePacked`) + `solidity_convert_expr.cpp` (multi-arg fold) | OVER + UNDER | Multi-arg fold via `_ESBMC_abi_fold` (post-T2.1, 2026-04-30): all uint256-castable args (including descended value-tuple components) chain through `acc' = acc * FNV_PRIME + next`, so changing any folded arg changes the encoded result. `t_bytes_*` / `t_bytesN` / function-decl / type-only-tuple args are still skipped. | Packed byte-layout properties (selector presence, delimiters, ABI-spec padding) cannot be verified — fold is identity-style, not bit-accurate. | (a) Multi-arg encodings whose differing args are bytes/bytesN — tracked by `abi_encode_bytes_arg_distinct_pass_knownbug_b7c` (B7-C follow-up). (b) Function-selector dispatch where the only difference is the selector itself (e.g. `encodeWithSignature("f1()")` vs `encodeWithSignature("f2()")`) — the signature literal lowers to a string-cast-to-uint256 which folds, but `encodeCall(this.f, ())` skips its function-ref arg entirely. |
