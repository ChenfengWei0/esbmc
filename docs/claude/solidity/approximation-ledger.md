...
| 16 | `revert` / `require` / failed `.transfer()` | `solidity_convert_modifier.cpp::build_revert_rollback_block` (B1) + `solidity_convert_stmt.cpp::TryStatement` if/else nondet split | UNDER for state, OVER for control flow | Public/external reverts run B1's `*this = _sol_save_this; return;`. Internal/private reverts and transfer-failure paths still emit `__ESBMC_assume(false)`. Try/catch is modelled as `if (nondet) { call; success } else { catch }` — catch sits in the else branch so SSA branching keeps success-arm writes out of catch's view (T2.2 confirmed empirically). | None for try/catch state rollback (closed by B1 + SSA branching, regression-locked by `try_catch_*_rollback_pass`). | (a) Cross-iteration leak in dispatcher harness when an internal-helper revert via legacy `assume(false)` leaves state writes on a sibling iteration's path — tracked as B5-B follow-up. (b) Standalone `addr.transfer()` revert outside try/catch still uses `__ESBMC_assume(false)` so a path-pruned write would survive into other paths via SSA merge if the harness iterates. |
