#!/usr/bin/env python3
"""Summarize VeriPUT unit certification journals and emit retry schedules.

The journal is produced by `unit_schedule_run.py`.  This script is read-only
unless `--out` or `--retry-out` is passed; it never invokes solc, Forge,
fuzzing, ESBMC, or certification jobs.
"""
...
def _retry_schedule(schedule: dict, latest: dict[str, dict]) -> dict:
    ...
    return {
        ...
        "shard": schedule.get("shard"),
        "limit": schedule.get("limit"),
        "cert_out": schedule.get("cert_out"),
        "summary": {
            "jobs": len(retry_jobs),
        ...
        "duplicate_jobs": schedule.get("duplicate_jobs") or [],
        "jobs": retry_jobs,
    }
...
def summarize(journal: str, *, schedule_path: str = "", sample_limit: int = 10) -> dict:
    ...
    by_priority_latest = defaultdict(Counter)
    ...
    if schedule:
        schedule_jobs = {
            job.get("job_id"): job
            for job in schedule.get("jobs") or [] if job.get("job_id")
        }
    ...
    for job_id, row in latest.items():
        status = row.get("status") or "<missing-status>"
        job = schedule_jobs.get(job_id) or {}
        bench = row.get("benchmark") or job.get("benchmark") or "<unknown>"
        ...
        by_priority_latest[priority][status] += 1
        if status != "ok" and len(samples[bucket]) < sample_limit:
            samples[bucket].append({
                "job_id": job_id,
                "benchmark": bench,
                "subject_id": row.get("subject_id") or job.get("subject_id"),
                "contract": row.get("contract") or job.get("contract"),
                "unit": row.get("unit") or job.get("unit"),
                "status": status,
                "reason": row.get("reason"),
                "stdout_tail": (row.get("stdout_tail") or "")[-800:],
    ...
    never_attempted = 0
    if schedule:
        never_attempted = len([job_id for job_id in schedule_jobs if job_id not in latest])

    return {
        "schema": "veriput-unit-journal-summary/v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "journal": journal,
        ...
        "by_priority_latest": {
            ...
            for priority, counter in sorted(by_priority_latest.items())
        },
...
def main() -> int:
    ...
    ap.add_argument("journal", help="unit_schedule_run.py JSONL journal")
    ap.add_argument("--schedule", default="", help="optional original unit schedule JSON")
