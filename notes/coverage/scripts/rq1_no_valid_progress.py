...
This script is the single source for N/205 progress reporting.  It reads the
per-case root-cause TSV and computes category counts plus patch coverage from
hard-coded patch batches.  Do not report N/205 from memory.
"""
...
DEFAULT_PEER_DATASET_ROOT = Path("/home/samson/workspace/VeriPUT/Datasets/Peer-Reviewed-Contracts")
...
def _safe_subject_dir_name(text: str) -> str:
    keep = []
    for ch in str(text):
...
HARD_REQUIREMENTS = (
    "Do not report N/205 from memory; run this script or quote its last output.",
    "Every user-facing progress update must include theoretical_progress=N/205.",
    ...
    "theoretical repair N/205, and actual RQ1 valid/PUT/R1R2 progress.",
...
SUBAGENT_PLAN = (
    ("A01", "goto_coverage no-claims/cov-report emission", "src/goto-programs/goto_coverage.cpp",
     "3/204 micro patch"),
    ...
    ("A03", "Solidity frontend call/selector/modifier fallback",
     ...
     "src/solidity-frontend/solidity_convert_modifier.cpp", "ESBMC no-cov slice"),
    ...
    ("A07", "NOT-CERTIFIED CE-to-region mismatch", "scripts/solidity_path_generalise.py",
     "3/204 micro patch"),
    ("A08", "certify_all witness extraction and partial journals",
     "notes/coverage/scripts/certify_all.py", "NOT-CERTIFIED slice"),
    ("A09", "path-cov-max-goals split/fallback", "notes/coverage/scripts/certify_all.py",
     "2/204 small bucket"),
    ("A10", "NO-PATH bounded-holds retry/unwind", "notes/coverage/scripts/certify_all.py",
     "3/204 small bucket"),
    ...
    ("A12", "Stage4 PUT materialization/accounting",
     "notes/coverage/scripts/put_all.py,scripts/solidity_path_put.py", "no-PUT/no-R1R2 support"),
    ("A13", "R1 oracle ladder strengthening", "scripts/solidity_path_put.py",
     "PUT quality support"),
    ("A14", "R2 oracle ladder and fuzz-refute integration", "scripts/solidity_path_put.py",
     "PUT quality support"),
    ("A15", "unit scheduling priority and state/env coords",
     "notes/coverage/scripts/unit_schedule.py", "runner support"),
    ("A16", "runner Stage2 budget/Stage4 reserve", "notes/coverage/scripts/rq1_veriput_run.py",
     "56/204"),
    ...
    ("A19", "remote worker/results sync automation", "notes/coverage/scripts/rq1_remote_pump.py",
     "resource utilization"),
    ("A20", "RQ1 progress ledger/audit script", "notes/coverage/scripts/rq1_no_valid_progress.py",
     "reporting integrity"),
    ("A21", "target manifest / subject discovery", "notes/coverage/scripts/target_manifest.py,"
     "notes/coverage/scripts/veriput_subjects.py", "schedule small bucket"),
    ("A22", "VeriPUT recipe option modeling", "notes/coverage/scripts/veriput_recipe.py",
    ...
    ("A24", "final integration reviewer",
     "all touched files, no independent writes unless resolving conflicts", "integration"),
    ("A25", "historical result adoption canonicalization",
     "notes/coverage/scripts/rq1_results_adopt.py", "peer historical adoption"),
    ("A26", "remote pump build/sync stability", "notes/coverage/scripts/rq1_remote_pump.py",
     "resource utilization"),
    ("A27", "certify CE/region mismatch retry", "notes/coverage/scripts/certify_all.py",
     "certify-not-certified"),
    ("A28", "PUT R1/R2 quality strengthening", "scripts/solidity_path_put.py",
...
FIXED_MICRO_PATCH_CATEGORIES = {
    ...
    "constructor-decl-duplicate-state-name": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "a03-modifier-selector-call-fallback": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "solidity-bytes-string-mapping-fallback-a04": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "a06-migrate-value-set-tuple-residual": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "solidity-tuple-ref-fallbacks": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "a29-solidity-type-meta-param-context": "ESBMC_NO_COV_REPORT_FRONTEND_OR_COVERAGE",
    "a15-unit-schedule-cheap-first-state-coords": "RUNNER_FIRST_UNITS_CONSUMED_SUBJECT_BUDGET",
    "a11-no-coordinate-subject-fallback": "CERTIFY_NO_COORDINATE_NO_PUT_REGION",
    "a09-path-cov-goal-cap-split-fallback": "CERTIFY_PATH_COV_GOAL_CAP_REFUSAL",
    "a21-target-manifest-subject-discovery": "SCHEDULE_NO_TARGET_UNITS",
    "a10-no-path-bounded-holds-retry": "CERTIFY_NO_PATH_OR_BOUNDED_HOLDS_EMPTY",
    "a30-small-no-valid-runner-materialization": "UNCLASSIFIED_RESULT_SCHEMA_OR_ARTIFACT_MISSING",
}
...
PATCH_BATCHES = (
    PatchBatch(
        ...
        deadline_hours=2.0,
        owner="main-agent",
        fix_target=("notes/coverage/scripts/rq1_veriput_run.py: Stage2 cap, "
    ...
    PatchBatch(
        ...
        deadline_hours=3.0,
        owner="main-agent",
        fix_target=("notes/coverage/scripts/rq1_veriput_run.py: no-output and "
    ...
    PatchBatch(
        ...
        fix_target=("src/solidity-frontend/* and src/goto-programs/goto_coverage.cpp: "
    ...
    PatchBatch(
        ...
        deadline_hours=8.0,
        owner="subagents+main-review",
        fix_target=("notes/coverage/scripts/certify_all.py and "
    ...
    PatchBatch(
        ...
        deadline_hours=10.0,
        owner="subagents+main-review",
        fix_target=("veriput_subjects.py, certify_all.py, put_all.py, and "
    ...
    with stream:
        ...
        for row in reader:
            ...
            index[(bench, subject)] = {
                ...
                "subject_dir": (row.get("subject_dir") or row.get("path") or "").strip(),
            }
    return index
...
def schedule_status(countdown: dict, applied: set[str]) -> dict:
    ...
    for batch in PATCH_BATCHES:
        ...
        if elapsed_h >= batch.deadline_hours:
            ...
            upcoming.append({
                **row,
                "remaining_to_deadline_h":
                round(batch.deadline_hours - elapsed_h, 3),
            })
    status = "on_schedule"
...
def _subagent_template() -> list[dict]:
    ...
    for agent_id, task, write_scope, coverage in SUBAGENT_PLAN:
        agents.append({
            ...
            "task":
            task,
            "write_scope": [item.strip() for item in write_scope.split(",") if item.strip()],
            "expected_coverage":
            coverage,
            "mode":
            "write",
...
def init_subagent_state(path: Path, force: bool = False) -> dict:
    ...
    existing_by_slot = {
        ...
        for agent in existing.get("agents") or [] if isinstance(agent, dict)
    }
    agents = []
    ...
    doc = {
        ...
        "rules": {
            "write_scope":
            "subagents may edit only their exclusive write_scope",
            "progress":
            "only completed entries with non-empty patch_id count",
            "cross_review": ("Every write-mode completed patch starts as pending review and "
                             "must be inspected by an independent review/integration agent "
                             "before it is treated as fully integrated."),
            "analysis_required": ("Before editing, inspect prior failure artifacts plus owning "
            ...
            ("review_status=accepted is ignored for net theory unless the "
             ...
             "patch_commit."),
            "reasoning_effort": ("Spawn calls must explicitly request reasoning_effort=high; "
...
def record_subagent(
    ...
    for agent in doc.get("agents") or []:
        ...
        if note:
            agent["note"] = note
        if review_status:
            if review_status not in {"pending", "accepted", "rejected", "needs-work"}:
                raise SystemExit("--record-subagent-review-status must be one of "
...
def subagent_patch_review_sets(subagents: dict) -> dict[str, set[str]]:
    ...
    for agent in subagents.get("agents") or []:
        ...
        if mode != "readonly":
            ...
            commit_sha = str(
                agent.get("review_commit") or agent.get("commit_sha") or agent.get("commit")
                or agent.get("patch_commit") or "").strip()
...
def merge_subagent_docs(primary: dict, extra: dict) -> dict:
    merged = dict(primary)
    merged["agents"] = list(primary.get("agents") or []) + list(extra.get("agents") or [])
    return merged


def subagent_summary(subagents: dict) -> dict:
    agents = [agent for agent in subagents.get("agents") or [] if isinstance(agent, dict)]
    by_status = Counter(str(agent.get("status") or "unknown") for agent in agents)
    active_agents = [agent for agent in agents if agent.get("status") in {"leased", "running"}]
    dispatch_pending = [
        agent for agent in agents if str(agent.get("slot") or "").startswith("AUTO-") and agent.get(
            "status") in {"leased", "running"} and not str(agent.get("patch_id") or "").strip()
    ]
    write_patch_agents = []
    pending_review = []
    ...
    for agent in agents:
        ...
        if mode == "write" or (str(agent.get("patch_id") or "").strip() and
                               (agent.get("write_scope") or [])):
            write_patch_agents.append(agent)
            if str(agent.get("review_status") or "pending") != "accepted":
                pending_review.append(agent)
    return {
        "required_slots":
        DEFAULT_REQUIRED_SUBAGENTS,
        "min_active_required":
        DEFAULT_MIN_ACTIVE_SUBAGENTS,
        "defined_slots":
        len(agents),
        "active_running_or_leased":
        len(active_agents),
        "active_below_required_min":
        len(active_agents) < DEFAULT_MIN_ACTIVE_SUBAGENTS,
        "active_subagents_to_spawn_now":
        max(0, DEFAULT_MIN_ACTIVE_SUBAGENTS - len(active_agents)),
        "dispatch_warning": ("ACTIVE_SUBAGENTS_BELOW_MIN: spawn or reuse repair subagents now"
        ...
        sum(1 for agent in agents
            if agent.get("status") in {"running", "completed"} and agent not in dispatch_pending),
        "completed":
        by_status.get("completed", 0),
        "running":
        max(0,
            by_status.get("running", 0) - len(dispatch_pending)),
        "dispatch_pending_not_spawned":
        len(dispatch_pending),
        "dispatch_pending_slots": [agent.get("slot") for agent in dispatch_pending],
        "completed_write_patches_requiring_review":
        len(write_patch_agents),
        "pending_review_count":
        len(pending_review),
        "accepted_without_commit_count":
        len(accepted_without_commit),
        "accepted_without_commit_patch_ids":
        [agent.get("patch_id") for agent in accepted_without_commit[-20:]],
        "non_medium_active_count":
        len(non_medium_active),
        ...
        "pending_review_patch_ids": [agent.get("patch_id") for agent in pending_review[-20:]],
        "queued":
        by_status.get("queued", 0),
        "by_status":
        dict(sorted(by_status.items())),
        "capacity_configured":
        int(subagents.get("max_concurrent_threads_per_session") or 0),
    }
...
def is_historical_result_path(path: Path) -> bool:
    return any(marker in part for part in path.parts for marker in HISTORICAL_RESULT_MARKERS)
    ...
    for source in sources:
        ...
        for test in tests:
            if not isinstance(test, dict):
                continue
            if not _is_valid_reference_test(test):
                continue
            valid += 1
...
def _rejected_test_counts(source: dict) -> tuple[int, int, int]:
    tests = source.get("valid_tests")
    if not isinstance(tests, list):
        return 0, 0, 0
    valid = put_valid = r1r2 = 0
    for test in tests:
        if not isinstance(test, dict) or _is_valid_reference_test(test):
            continue
        valid += 1
        if test.get("kind") != "put":
            continue
...
def _strength_counts(row: dict, put_doc: dict | None = None) -> tuple[int, int, int]:
    ...
    if (isinstance(row.get("valid_tests"), list) or isinstance(put_doc.get("valid_tests"), list)):
        row_valid, row_put, row_r1r2 = _valid_strength_from_tests(row)
        put_valid, put_put, put_r1r2 = _valid_strength_from_tests(put_doc)
        return (
            max(row_valid, put_valid),
            max(row_put, put_put),
            max(row_r1r2, put_r1r2),
        )
    derived_valid, derived_put, derived_r1r2 = _valid_strength_from_tests(row, put_doc)
    row_rejected = _rejected_test_counts(row)
    put_rejected = _rejected_test_counts(put_doc)
    row_valid = max(0, _as_int(row, "valid") - row_rejected[0])
    put_valid = max(0, _as_int(put_doc, "valid") - put_rejected[0])
    v = max(row_valid, put_valid, derived_valid)
    pv = max(max(0,
                 _as_int(row, "put_valid") - row_rejected[1]),
             max(0,
                 _as_int(put_doc, "put_valid") - put_rejected[1]), derived_put)
    rv = max(
        max(0,
            _as_int(row, "valid_put_with_R1_or_R2") - row_rejected[2]),
        max(0,
            _as_int(put_doc, "valid_put_with_R1_or_R2") - put_rejected[2]),
        derived_r1r2,
    )
    return v, pv, rv


def _rq1_expected_targets(results_root: Path,
                          inventory: Path | None = None) -> dict[tuple[str, str], dict]:
    if inventory is not None:
        inventory_doc = load_json_file(inventory, {})
        rows = (inventory_doc.get("rows")
                if isinstance(inventory_doc.get("rows"), list) else [])
        expected = {}
        for row in rows:
            dataset = str(row.get("bench") or "")
            subject = str(row.get("subject") or "")
            if dataset and subject:
                expected[(dataset, subject)] = row
        if expected:
            return expected
    veriput_root = results_root.resolve().parents[2]
    doc = target_manifest.build_manifest(veriput_root, ["bugfix124", "peer182", "stress203"],
                                         "include")
    out = {}
    for row in doc.get("targets") or []:
        if row.get("status") != "ok":
            continue
        dataset = RQ1_BENCHMARK_TO_DATASET.get(row.get("benchmark"))
        subject = row.get("subject_id")
        if dataset and subject:
            out[(dataset, subject)] = row
    return out


def actual_rq1_progress(results_root: Path,
                        inventory: Path | None = DEFAULT_INVENTORY) -> dict:
    total = valid = put = r1r2 = 0
    by_dataset: dict[str, Counter[str]] = {}
    skipped = Counter()
    expected = _rq1_expected_targets(results_root, inventory)
    peer_expected = {subject for (dataset, subject) in expected if dataset == "peer182"}
    ...
    for result in sorted(results_root.glob("*/subjects/*/result.json")):
        ...
        if key not in expected:
            ...
            for expected_key in expected:
                expected_dataset, expected_subject = expected_key
                if (expected_dataset == dataset
                        and _safe_subject_dir_name(expected_subject) == subject):
                    canonical = expected_key
                    break
            if canonical is None:
                skipped["unexpected_subject_result"] += 1
                if dataset == "peer182":
        ...
        if previous is not None:
            ...
            try:
                ...
                continue
        seen[key] = result
    for dataset, subject in sorted(expected):
        bucket = by_dataset.setdefault(dataset, Counter())
        total += 1
        bucket["total"] += 1
        result = seen.get((dataset, subject))
        if result is None:
            skipped["missing_expected_result"] += 1
            continue
        if dataset == "peer182":
    ...
    return {
        ...
        "peer182_missing_expected_subjects": sorted(peer_expected - peer_observed_expected),
        "peer182_missing_expected_count": len(peer_expected - peer_observed_expected),
...
def resource_maximization(
    ...
    if sub_summary["defined_slots"] < DEFAULT_REQUIRED_SUBAGENTS:
        reasons.append(f"subagent_plan_incomplete:{sub_summary['defined_slots']}/"
                       f"{DEFAULT_REQUIRED_SUBAGENTS}")
    if sub_summary["capacity_configured"] < DEFAULT_REQUIRED_SUBAGENTS:
        reasons.append("subagent_capacity_below_24:"
    ...
    if sub_summary.get("active_below_required_min"):
        reasons.append("active_subagents_below_min:"
            ...
            <= DEFAULT_MIN_ACTIVE_SUBAGENTS:
        reasons.append("active_subagents_at_minimum_margin:"
                       f"{sub_summary.get('active_running_or_leased', 0)}/"
                       f"{DEFAULT_MIN_ACTIVE_SUBAGENTS};prepare_dispatch_now")
    if sub_summary.get("dispatch_pending_not_spawned"):
        reasons.append("subagent_dispatch_pending_not_spawned:"
    ...
    capacity_parallel = int(
        worker.get("capacity_case_parallel") or (worker.get("capacity") or {}).get("case_parallel")
        or 0)
    if worker.get("started") and capacity_parallel > 0 \
            and case_parallel < capacity_parallel:
        reasons.append(f"remote_case_parallel_below_capacity:{case_parallel}/"
                       f"{capacity_parallel}")
    ...
    return {
        ...
        "subagents_to_spawn_now": sub_summary["active_subagents_to_spawn_now"],
...
def patch_coverage(
    ...
    for batch in PATCH_BATCHES:
        if batch.patch_id in applied:
            ...
            batch_rows.append({
                ...
                "counted":
                sum(counts.get(category, 0) for category in batch.categories),
            })
    ...
    details = {
        ...
        "micro_contributions":
        micro_rows,
        "dedupe_rule": ("micro patch coverage is counted only when its root-cause "
                        "category is not already covered by a completed batch patch"),
        "capped_at_denominator":
        DENOMINATOR,
    }
...
def _canonical_validation_feedback_rows(
    ...
    for (bench, subject), result in sorted(seen.items()):
        ...
        include_no_valid = (no_valid_since_ts is not None and result_mtime >= no_valid_since_ts)
...
def _feedback_suggested_scope(row: dict) -> list[str]:
    ...
    original = str(row.get("original_category") or row.get("root_cause_category") or "")
    if bucket == "ORACLE_ACCOUNTING_METADATA_LOST":
        return [
            "notes/coverage/scripts/rq1_veriput_run.py", "notes/coverage/scripts/put_all.py",
...
def sync_feedback_repair_tickets(path: Path, rows: list[dict]) -> int:
    ...
    for row in rows:
        ...
        ticket = {
            ...
            row.get("subject"),
            "category":
            row.get("original_category") or row.get("root_cause_category") or row.get("category"),
            "result_bucket":
            row.get("category"),
            "valid":
            ...
            "r1r2":
            int(row.get("r1r2") or 0),
            "priority":
            "high" if str(row.get("category") or "").startswith("ESBMC_") else "normal",
            "trigger_reason": ("canonical-result-below-valid-put-r1r2-after-progress-start"),
            "subject_dir":
            row.get("subject_dir"),
            "result_file":
            row.get("result_file"),
            ...
            ("If this case belongs to an already-covered category and is "
             ...
             "quality progress."),
            "subagent_rule": ("Inspect listed failure records and owning source code before "
...
def validation_feedback(
    ...
    covered_micro_categories = {
        category
        for patch_id, category in FIXED_MICRO_PATCH_CATEGORIES.items()
    ...
    canonical_rows = _canonical_validation_feedback_rows(results_root, root_cause_index,
                                                         no_valid_since_ts)
    latest_status = _canonical_latest_status(results_root)
    tickets_written = (sync_feedback_repair_tickets(repair_tickets, canonical_rows)
    ...
    for row in rows:
        ...
        original_category = str(
            row.get("original_category") or row.get("root_cause_category") or "")
        ...
        if not valid and claim_category:
            ...
            no_r1r2[key] = item
    invalidated = sorted(no_valid.values(), key=lambda item: (item["bench"], item["subject"]))
    ...
    return {
        ...
        str(results_root),
        "claim_categories":
        sorted(claim_categories),
        "no_valid_invalidated_count":
        len(invalidated),
        "no_put_quality_debt_count":
        len(no_put),
        "no_r1r2_quality_debt_count":
        len(no_r1r2),
        "actual_valid_no_put_count":
        actual_no_put,
        "actual_valid_no_r1r2_count":
        actual_no_r1r2,
        "quality_patch_claimed":
        quality_claimed,
        "quality_patch_ids_applied":
        sorted(applied & quality_patch_ids),
        "canonical_feedback_ticket_sync": {
            "enabled":
            sync_tickets,
            "tickets_written":
            tickets_written,
            "rule": ("Canonical result.json rows below valid PUT R1/R2 and written "
        ...
        "put_theoretical_progress_gross": {
            ...
            "denominator":
            valid_cases,
            "rule": ("If a PUT quality patch is applied, gross assumes valid "
        ...
        "r1r2_theoretical_progress_gross": {
            ...
            "denominator":
            valid_cases,
            "rule": ("If an R1/R2 quality patch is applied, gross assumes valid "
        ...
        "quality_debt_examples": {
            "no_put": sorted(no_put.values(), key=lambda item:
                             (item["bench"], item["subject"]))[:40],
            "no_r1r2": sorted(no_r1r2.values(), key=lambda item:
                              (item["bench"], item["subject"]))[:40],
        },
        "rule": ("Each unique covered-category subject with a later no-valid repair "
...
def main() -> int:
    ...
    parser.add_argument("--extra-subagents", type=Path, default=DEFAULT_EXTRA_SUBAGENTS)
    ...
    parser.add_argument("--repair-tickets", type=Path, default=DEFAULT_REPAIR_TICKETS)
    parser.add_argument("--case-theory-claims", type=Path, default=DEFAULT_CASE_THEORY_CLAIMS)
    ...
    primary_subagents = load_json_file(
        args.subagents, {
            "status": "missing",
            "agents": [],
    ...
    remote_state = load_json_file(
        args.remote_state, {
            "status": "missing",
            "note": "remote state file missing; remote runner is not hard-confirmed",
        })
    local_state = load_json_file(
        args.local_state, {
            "status": "missing",
            "note": "local state file missing; local runner is not hard-confirmed",
        })
    remote_live = ({"skipped": True} if args.no_remote_probe else remote_probe(args.remote_host))
    actual = actual_rq1_progress(args.results_root, args.inventory)
    resources = resource_maximization(subagents, remote_state, remote_live, local_state)
    ...
    covered_gross, covered_categories, coverage_details = patch_coverage(counts, applied)
    ...
    case_claim_count = len({(str(claim.get("bench") or ""), str(claim.get("subject") or ""))
                            for claim in case_claims})
    if case_claim_count:
        covered_gross = min(DENOMINATOR, covered_gross + case_claim_count)
        implemented_gross = min(DENOMINATOR, implemented_gross + case_claim_count)
        case_rows = [{
    ...
    feedback = validation_feedback(args.repair_tickets, args.results_root, root_cause_index,
                                   set(covered_categories), applied, actual,
                                   float(state.get("start_ts") or 0.0), True)
    ...
    implemented_net = max(
        0,
        implemented_gross - int(implemented_feedback.get("no_valid_invalidated_count") or 0),
    )
        ...
        json.dumps(
            {
                ...
                "rule": ("completed write-mode patch_ids count toward net theoretical "
                         "coverage only when review_status=accepted. Pending patch_ids are "
                         "reported as implemented/provisional but do not raise net N/205; "
                         "rejected/needs-work patch_ids are removed from both provisional "
                         "and fully-integrated counts."),
            },
            indent=2,
            sort_keys=True))
    print(f"implemented_progress_provisional_gross={implemented_gross}/"
          f"{DENOMINATOR}")
    print(f"implemented_progress_provisional={implemented_net}/"
          f"{DENOMINATOR}")
    ...
    print("put_theoretical_progress_gross="
          f"{put_gross['numerator']}/{put_gross['denominator']}")
    print("put_theoretical_progress="
          f"{put_net['numerator']}/{put_net['denominator']}")
    print("r1r2_theoretical_progress_gross="
          f"{r1r2_gross['numerator']}/{r1r2_gross['denominator']}")
    print("r1r2_theoretical_progress="
    ...
    print(
        json.dumps(
            {
                "default_mode":
                "write_allowed_only_inside_exclusive_write_scope",
                "readonly_mode":
                "only when mode=readonly in status file",
    ...
    for batch in PATCH_BATCHES:
        ...
        print(f"  {batch.patch_id}: covers={batch_count}, "
