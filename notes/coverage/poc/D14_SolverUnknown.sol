// MINIMAL REPRODUCTION, reduced automatically from ../inputs/st1inch__St1inch.flat.sol
// against the predicate `solver-unknown` at --solidity-max-tx 1, --memlimit 4g,
// timeout 120s, backend flags --z3 --tuple-node-flattener,
// focus-function setFeeReceiver, solc /home/samson/.solc-select/artifacts/solc-0.8.23/solc-0.8.23.
// The backend flags are part of the reproduction: the same query fails
// differently under different backends, so a run without them is a run of
// something else.
// Every element still here is one whose removal made the failure GO AWAY,
// so this file is not merely smaller -- each remaining part is load-bearing.
// CHECKPOINT: PASS fn, 4355 lines. This file is the reduction AS OF
// THAT POINT, written after every accepted removal because a run
// this long is expected to be killed -- a single write at the end is
// the one moment that may never arrive. It reproduces the predicate;
// it is simply not known to be MINIMAL.
