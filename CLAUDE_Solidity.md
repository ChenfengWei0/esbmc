...
### Resolved Bugs (2026-03-31 / 2026-04-22)
...
fusion-protocol. Bug 10 fixed 2026-04-16 (KNOWNBUG promotion). Bugs
11-15 fixed 2026-04-17/18 (mapping storage-sharing work). Bug 16
fixed 2026-04-20 (VSA ASSUME + library null-pointer contracts).
Bugs 17-18 fixed 2026-04-22 after the SolidiFi full-50 rerun
(v4 exposed two frontend gaps hit by real SafeMath-using contracts).
Bug 19 fixed 2026-04-22 (incidental `map_generic_set` size-arg typo
uncovered while tracing bug 18).
Summary:
...
| **11** | `std::out_of_range` crash when reading a nested-mapping public getter (`c.m(k1, k2)` where `c = new C()`) | `get_new_mapping_index_access` hit its struct-shaped `val_flg == "generic"` branch with an empty identifier because the nested-mapping leaf type arrives as `MAPPING`, so the later `prefix.length()` substring on the empty id threw | `solidity_convert_mapping.cpp::get_new_mapping_index_access`: early-return before the struct path when the leaf value type itself is `MAPPING`. Regression: `nested_mapping_pubgetter_compile` (commit `965a2a4c2a`). |
| **12** | Nested mapping writes (`m[k1][k2] = v`) and reads (`m[k1][k2]`) landed in different slots in `is_new_expr` mode — assertions never held even when the write and read used the same keys | Under the singleton `mapping_t` model, the chained `index_exprt` on the declared `array_typet` subtype used one slot per level, but the public-mapping-getter read path folded keys independently and indexed through the function-call boundary where `map_generic_get` returns a `void*` pointer that doesn't carry the write's value through | `solidity_convert_mapping.cpp`: new helper `combine_mapping_keys_256` packs xor-folded 64-bit keys into 64-bit lanes of a uint256. `get_index_access_expr` intercepts nested-mapping IndexAccess early in `is_new_expr` mode and routes both GET/SET through a single `map_<leaf>_get/set` call against the combined key, so writes and reads share the same slot. Regressions: `nested_mapping_in_contract_pass/fail`, `cross_nested_mapping_pass/fail` (commit `965a2a4c2a`). |
...
| **17** | A Solidity user function whose name collided with a C stdlib.h export (`div`, `abs`, `malloc`, `atoi`, `sort`, …) was silently hijacked by the C decl.  SafeMath.ceil's internal `div(d, m)` call resolved to `c:@F@div` returning `div_t` struct, and the downstream `mul(..., m)` then tripped "got struct, expected unsignedbv" at the SafeMath.mul `a` param type-check.  Appeared on SolidiFi buggy_28 / 33 / 38 / 42 (150 pairs) as 0-TOD / ALL-ERR in the v4 50-case rerun. | `get_sol_builtin_ref`'s FunctionCall branch unconditionally looked up `c:@F@<name>` in the C symbol table and bound to it when found, without checking whether the Solidity Identifier already had a positive `referencedDeclaration` pointing at a user decl. | `solidity_convert_ref.cpp::get_sol_builtin_ref`: early-return when the callee has `referencedDeclaration > 0`.  Exception for `__ESBMC_assume` / `__ESBMC_assert` / `__VERIFIER_assume` / `__VERIFIER_assert` — user contracts commonly declare stub versions so that solc can compile standalone, and ESBMC intentionally shadows them with the real intrinsic at verification time via this same code path.  Regressions: `user_name_shadows_c_stdlib_pass/fail` (commit `2614c484c2`). |
| **19** | `map_generic_set(&m, k, v, sz)` was called with `v` in the `sz` slot (struct-typed value where `size_t` expected).  Worked for small structs that implicitly coerced (mapping_12-style `struct{uint8}`) but errored "got struct, expected unsignedbv" once the struct had enough fields to defeat the implicit cast (buggy_24's 18-field `Record`).  Discovered incidental to bug 18 while walking `is_new_expr` struct-valued mapping writes. | `get_new_mapping_index_access`'s `val_flg=="generic"` SET branch at `solidity_convert_mapping.cpp` pushed `symbol_expr(added_sym)` into both the value and size argument slots of the call; the `size_of_expr` local was constructed but never pushed. | `solidity_convert_mapping.cpp`: replace the duplicate value push with `size_of_expr`.  Regression: `map_large_struct_set_pass/fail` — 9-field struct routed through `new Store()` so the emission actually fires, demonstrating the typo was semantic (fails frontend type-check without fix, passes with it). |
| **18** | `mapping(K => T[N])` — mapping whose value is a **fixed-size** array — tripped "unsupported mapping value type: sol_type=ARRAY_LITERAL" in the `is_new_expr` dispatch.  Sibling gap to the dynamic-array (`T[]`) fix in bug 11/12.  SolidiFi buggy_24 (FomoFeast) has three `mapping(address => Record[9])` state vars and all 63 of its candidate TOD pairs errored on conversion in the v4 50-case rerun. | `get_new_mapping_index_access` had branches for DYNARRAY (`T[]`), STRUCT, MAPPING, and scalar leaves, but no branch for ARRAY_LITERAL (`T[N]`).  The value fell into the generic path and errored at the `val_sol_type` switch. | **C helper** — new `map_fixed_arr_get(m, k, sz)` in `solidity_mapping.c`.  Lazy `calloc(1, sz)` on first access gives Solidity's pre-bound-to-zero-filled-slab semantics and returns the same pointer on subsequent reads (so element writes via `[i]` persist).  Differs from `map_dynarr_get` (which returns NULL for unwritten keys) — fixed-size reads must not NULL-deref. **Frontend** — `solidity_convert_mapping.cpp::get_new_mapping_index_access`: `ARRAY_LITERAL` dispatch emits the helper call with `sizeof(T[N])` as the third arg and returns the raw `void*` (downstream `[i]` lowering handles slab indexing).  Set path (whole-array assign `m[k] = ...`) not supported yet — errors cleanly on absent `map_fixed_arr_set`.  Regressions: `map_fixed_array_value_pass/fail` (commit `6ae0e57c3f`). |
...
To really close this requires **heap-allocation-site sensitivity in VSA** — each `_ESBMC_alloc_array` call context should produce a distinct `dynamic_object` instance. That is a VSA architectural change (instrument allocators with call-site tags, threaded through `get_reference_set` + `assign_rec`), separate in scope from the per-constant-index scheme landed in `41a3b218f5`. Until then, `_ESBMC_arrcpy_2d` is truly load-bearing — the helper-frame local pointer is the only abstraction that gives VSA a fresh allocation identity per clone call.

Alternative partial mitigation: inline `_ESBMC_alloc_array` at the walker level (emit `size_t *block = calloc(1, total); block[0] = count;` inline at each allocation site in the walker) so each alloc has its own irep2 dynamic_object. Would work but leaks library internals into the frontend — trade-off not obviously better than keeping the helper.
...
#### F.3. Mapping of Fixed-Size Arrays (`mapping(K => T[N])`) — ✅ Fixed (2026-04-22)

Third distinct mapping-of-array shape (after F.1 `mapping(K=>V)[N]` and F.2 `mapping(K => V[])`): outer mapping, inner **fixed-size** array (e.g. `mapping(address => InvestRecord[9])`). Common in staking/investment contracts. First hit while running SolidiFi `buggy_24` (FomoFeast) during the v4 full-50 rerun.
...
**Frontend dispatch:**
- `solidity_convert_mapping.cpp::get_new_mapping_index_access`: new `val_sol_type == ARRAY_LITERAL` branch sets `val_flg = "fixed_arr"`, emits `map_fixed_arr_get(&array, pos, sizeof(T[N]))`, and returns the raw `void*` call (downstream `[i]` lowering casts to element type and indexes).
- Only the GET path is implemented. A whole-array assign `m[k] = new T[N](...)` would error at `map_fixed_arr_set` symbol lookup (not implemented yet — no real contract hits this pattern in the current benchmark set).

**Scope:** this fix covers the `should_treat_as_new` dispatch used by the TOD harnesses (which always run `new C()`). The non-new path for plain static-contract verification still uses the native nested-array `index_exprt` SMT model, which has its own encoding quirks with finite inner dimensions (2-bit-indexed inner array against 64-bit folded outer key); not exercised by the current benchmarks and left alone.

**Regression coverage:** `map_fixed_array_value_pass/fail` (CORE). Both use the `new Store()` pattern to force `is_new_expr` → my helper path. Pass confirms write-then-read coherence; fail confirms distinct keys don't alias.

**Incidental fix (2026-04-22):** `get_new_mapping_index_access`'s `map_generic_set` emission for struct-valued mappings (`mapping(address => User)` whole-struct writes) had a pre-existing typo where the size slot got the value symbol instead of `sizeof(value)` — `call.arguments().push_back(symbol_expr(added_sym))` in both the value and size slots. This worked by accident for small structs that implicitly coerced to `unsignedbv` in argument conversion (mapping_12-style `struct { uint8 age }`) but errored "got struct, expected unsignedbv" on larger structs (buggy_24's 18-field `User`). Fixed by replacing the second push with `size_of_expr`. Regression coverage: `map_large_struct_set_pass/fail` (both route through `new Store()` → `is_new_expr` → `map_generic_set` emission with a 9-field struct).

**Why not found earlier:** no existing regression test exercised the `mapping(K => T[N])` shape with `is_new_expr = true`. SolidiFi buggy_24's three `mapping(address => Record[9])` state vars are the first inputs to hit this combination (63 of its candidate TOD pairs errored on conversion in the v4 run; all 63 now progress to verify).
...
| ~~**Nested mapping key packing**~~ | ✅ Fixed (2026-04-17): `combine_mapping_keys_256` packs xor-folded 64-bit keys into 64-bit lanes of a uint256 so `m[k1][k2]` reads/writes hit a single `map_<leaf>_get/set` slot. Up to 4 levels fit losslessly; >4 levels still collapse but additional collisions are acceptable. This replaces the previous chain of `index_exprt` on the declared array subtype, which broke when the read path crossed a function-call boundary (`map_generic_get` returns a `void*` pointer that doesn't propagate the write). | `solidity_convert_mapping.cpp::combine_mapping_keys_256` + `get_index_access_expr` nested intercept |
...
### Test Baseline (2026-04-17)

**684 total tests** (2026-04-17): 682 pass, 2 pre-existing failures unchanged across recent Solidity frontend work (`mapping_17` — THOROUGH timeout, `unbound_11` — unrelated). 0 new regressions. Test flags: always use `--unwind N --no-unwinding-assertions` for bounded verification; omitting `--unwind` causes OOM on the SMT solver.

Growth from 607 → 684 (2026-04-14 → 2026-04-17) reflects `tod_auto_*`, `tod_balance_*`, `tod_counter_*`, `tod_disjoint_*`, `tod_two_cname_mapping_fail` TOD harness tests, the `nested_mapping_*` / `cross_*_mapping_*` mapping-path regression tests added while stabilising the nested-mapping and cross-contract storage paths for TOD detection, and `tod_bind_polymorphism_mapping_pass/fail` exercising the per-pointer `$bind` shadow refactor. After the fixes below, the full libsolidity stress sweep (1663 single-source files) shows **0 frontend crashes**.
...
| `tod_balance_pass` | — | ETH-balance TOD detection, non-racing |
| `tod_balance_fail` | — | ETH-balance TOD detection, racing `transfer/send` |
...
| `tod_bind_polymorphism_mapping_pass` / `_fail` | n/a | `A1 alias = A1(address(c2))` — per-pointer bind shadow + shadow-propagation + polymorphic getter read. Pre-fix would invert the verdicts. |
| `nested_mapping_in_contract_pass` / `_fail` | n/a | Combined-key nested-mapping write+read inside a single contract |
| `nested_mapping_pubgetter_compile` | n/a | Nested public-getter compiles (crash-fix regression) |
| `cross_single_mapping_pass` / `_fail` | n/a | Single-level public-getter read lines up with in-contract write (raw key, no fold) |
| `cross_nested_mapping_pass` / `_fail` | n/a | Nested public-getter read lines up with in-contract write (fold + combine) |
