...
| 10 | Calldata array-of-bytes | *(limitation)* `bytes[] calldata` / `string[] calldata` indexing | OVER→UNPROVABLE | Array elements are fresh nondet BytesDynamic struct reads *not* routed through `llc_nondet_bytes`, so `length` / `initialized` of `a[i]` are fully symbolic. | All reads of `a[i][j]` spuriously trigger init/bounds checks → VERIFICATION FAILED. | Tests of this shape stay in `KNOWNBUG`. |
