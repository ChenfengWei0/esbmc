...
**Incidental fix:** while tracing the same path, the existing `map_generic_set` emission for struct-valued mappings (`mapping(address => User)` whole-struct writes) had a pre-existing typo where the size slot got the value symbol instead of `sizeof(value)`. This worked by accident for small structs (mapping_12-style `struct { uint8 age }`) but errored "got struct, expected unsignedbv" on larger structs (buggy_24's 18-field `User`). The fix is staged for a follow-up — buggy_24's flow now reaches verify via the fixed_arr path and doesn't need the struct-set pattern.
