...
// (goto-symex with2t chain on a 5-level mapping-of-X storage-ref
...
contract C {
    mapping(address => mapping(address => mapping(bytes32 => mapping(address => mapping(address => Balance))))) private _b;
    function dock(address app, bytes32 h, address inner, address[] calldata toks) external {
        for (uint256 i = 0; i < toks.length; i++) {
            Balance storage b = _b[msg.sender][app][h][inner][toks[i]];
