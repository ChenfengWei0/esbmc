// FULL aqua flat (88KB) regression pin — landed 2026-05-16.
// PURPOSE: guard the full aqua contract vs regression to the
// pre-2026-05-15 SIGABRT (bare smt_sort / value_set base_type_eq /
// with2t is_array_type on >=3-level nested mapping-of-struct). The
// 16-line minimal repro is cov_pilot_aqua_Aqua (CORE 75%); this pin
// proves the WHOLE contract runs to a coverage verdict after the
// solidity_convert_expr.cpp:4203 fix.
// MEASURED (new binary, 11 runs): 10/11 = Branches 164 / Reached 13 /
// Branch Coverage 7.926829268292683%; 1/11 cold outlier 166 (7.831%).
// Reached 13 = 11/11 stable. The 164<->166 wobble is a newly-surfaced
// S-D residual: denominator dedup keys on (condition,
// location.as_string()); under --no-assertions location collapses to
// "line 1" (R3) and complex conditions carry SSA temps
// (return_value$..$N), so duplicate copies occasionally fail to fold.
// CORE pins only stable signals (Reached:13, ~7% emitted); a SIGABRT
// regression makes the [Coverage] block vanish -> fail -> alarm.
// S-D hardening is a separate tracked stage. See
// notes/Results/branch_cov/SESSION_2026_05_16_RESULTS.md.
// SPDX-License-Identifier: LicenseRef-Degensoft-Aqua-Source-1.1
pragma solidity =0.8.30 >=0.4.16 ^0.8.0 ^0.8.20;
...
// node_modules/@1inch/solidity-utils/contracts/interfaces/IERC7597Permit.sol
