...
__ESBMC_HIDE:;
    ...
    b.length = len;
#define STEP(i) do { \
        if ((i) < copy_len) b.data[i] = (unsigned char)str[i]; \
        else if ((i) < len) b.data[i] = 0; \
    } while (0)
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    return b;
...
__ESBMC_HIDE:;
    ...
    b.length = new_len;
#define STEP(i) do { \
        if ((i) < new_len) b.data[i] = src->data[i]; \
    } while (0)
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    return b;
...
__ESBMC_HIDE:;
    BytesStatic r = {0};
#define STEP(i) if (a->length > (i)) r.data[i] = a->data[i] & b->data[i]
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    r.length = a->length;
    return r;
...
__ESBMC_HIDE:;
    uint256_t r = 0;
#define STEP(i) r = (b->length > (i)) ? ((r << 8) | b->data[i]) : r
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    return r;
}
...
__ESBMC_HIDE:;
    ...
    out.length = new_len;
#define STEP(i) do { \
        if ((i) < src->length) out.data[i] = src->data[i]; \
    } while (0)
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    return out;
...
__ESBMC_HIDE:;
    ...
    b.length = new_len;
#define STEP(i) do { \
        if ((i) < src->length) b.data[i] = pool->pool[src->offset + (i)]; \
    } while (0)
    _ESBMC_BS_UNROLL_32(STEP);
#undef STEP
    return b;
