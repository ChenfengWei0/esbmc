      ...
      if (i_node.contains("nodes"))
      {
        for (auto i : i_node["nodes"])
        {
            ...
            !i["name"].empty())
          {
            ...
            // e.g.
            // contract A {function func_1() public virtual returns (int8) {} }
            // contract B is A {function func_1() public override pure returns (int8) {} }
            // for contract B, we should not merge A's func_1 ast, otherwise we will have conflict
...
bool solidity_convertert::move_initializer_to_ctor(
  ...
  std::string ctor_id)
{
  log_debug("solidity", "moving initializer to the constructor");
  if (ctor_id.empty())
  {
...
{
  log_debug(
    "solidity", "moving inherited members to the constructor {}", ctor_id);

  std::string this_id = ctor_id + "#this";
  if (context.find_symbol(this_id) == nullptr)
  {
    log_error("cannot find this pointer {}", this_id);
  }
