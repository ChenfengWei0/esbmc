  ...
  {"Coverage options",
   {
     ...
     {"path-cov-certify",
      ...
      "exit is REFUTED whenever the box is certified, so a certified run prints "
      "VERIFICATION FAILED. RESULT: CERTIFIED means every input in the box "
      "walks that path; REFUTED gives a counterexample input that is inside the "
      "box but leaves the path, which is exactly the witness needed to shrink "
      "the box; VACUOUS means the box admits no execution reaching that path at "
