...
- **Array-of-struct-with-mapping / array-of-nested-fixed-array**: the walker's non-scalar path unrolls a compile-time per-element copy and recurses into each element, so mapping `.addr` retargeting reaches array-nested mappings. Multi-dim fixed arrays trip a separate ctor-side limitation (below).
- **Multi-dim fixed arrays** (`uint256[M][N]`): **KNOWNBUG** at the ctor layer, not the clone layer. `new C()` calloc's only the outer pointer array; the inner M-row buffers are never allocated. Writes through uninitialised inner pointers succeed under `--no-standard-checks` but reads from a different instance (or the same instance after re-dispatch) return nondet. The clone walker's per-element recurse correctly tries to `_ESBMC_arrcpy` each inner row but cannot recover from base's missing storage. See `esol_clone_multi_dim_knownbug`. Fix requires extending contract construction to recursively allocate nested pointer-backed state-var fields.
