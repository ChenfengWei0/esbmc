...
cmake -DESBMC_REGRESS_TIMEOUT=90 ..
...
- **Multi-dim fixed arrays of scalar leaves** (`uint256[M][N]`): fully isolated. The walker calls the library helper `_ESBMC_arrcpy_2d(base->grid, N, M, sizeof(elem))` as a SINGLE function call, which internally memcpy's the outer pointer array then element-copies each inner row into a fresh allocation. Why a single helper rather than frontend-emitted `c->grid = alloc_array(N, 8); c->grid[i] = arrcpy(base->grid[i], M, elem_sz)`? The per-slot emission broke symex's value-set tracking — successive index writes to a freshly-reassigned pointer field didn't flow through to subsequent reads (see `esol_clone_multi_dim_pass` header for the full delta-debug + disproved hypotheses, and `repro_raw/` for byte-identical raw-C/C++ equivalents that confirmed the backend handles the goto pattern correctly on its own).
...
- **Dynamic arrays** (1D `uint256[]`): isolated per-instance.  State-var dynarrays live in a global SMT array keyed by `_ESBMC_dynarr_idx(this->$address, idx)` (addr-replicated XOR fold, src/c2goto/library/solidity/solidity_array.c), so two `new C()` instances index disjoint slots.  Length lives in a parallel `<arr>_dynarray_len` infinite array keyed directly by `this->$address`.  The clone walker (solidity_convert_constructor.cpp:987-1110) emits a length copy plus a `--unwind`-bounded element-copy loop that re-keys base's slots into clone's.  See `esol_clone_dynamic_array_pass`, `dynarray_*_pass`.
- **Dynamic arrays** (2D `uint256[][]`): per-instance outer-slot isolation via the same fold; inner row is heap-pointer-backed.  Symbolic-size `new uint256[](size)` zero-inits the inner data (typed-element loop in `_ESBMC_alloc_array_sym`, src/c2goto/library/solidity/solidity_array.c).  Clone walker detects pointer-typed `elem_type` in the per-element copy and emits `_ESBMC_arrcpy` for a fresh inner-row allocation + element-copy.  See `dynarray_2d_two_instance_isolation_pass`, `dynarray_2d_clone_post_mutation_pass`.  Inner-row `arr[i].length` reads remain a pre-existing gap (not addressed in T1.1).
- **Dynamic arrays** (3D `uint256[][][]`): NOT covered.  The clone walker's pointer-elem `_ESBMC_arrcpy` is 1-level only — middle-row pointers still bit-copy.  3D state-var dyn-array isolation requires a recursive walker, deferred (CVC5 also rejects 3D state-var dyn-array encoding with `array select operating on non-array` even with `--cvc5-native-tuples`).
- **Structs of primitives**: copied (like scalars). Isolation holds via struct-level value copy.
- **Structs containing nested fixed arrays** (e.g. `struct Box { uint256[N] cells; }`): isolated. The Phase 2 ctor walker (`emit_ctor_deep_init_fixup` in src/solidity-frontend/solidity_convert_constructor.cpp around line 1297) recursively calloc's nested pointer-backed fields during contract construction, so `base.bx.cells` is a valid buffer before any read or copy. The clone walker then handles the inline `bx` recursion and emits `_ESBMC_arrcpy(base->bx.cells, N, sizeof(uint256))` per field. See `esol_clone_struct_array_pass` (CORE).
